...
Public Class BankCash_G
    ...
    Private Sub CalcBalSub(i As Integer)
        If Not lop AndAlso Val(G.Rows(i).Cells(GC.SubAccNo).Value) > 0 Then
    ...
    Sub WhatsApp()
        ...
        Try
            G.CurrentCell = G.Rows(0).Cells(GC.SubAccNo)
        Catch ex As Exception
            Return
        End Try
        Dim fileName As String = bm.GetNewTempName(Val(G.CurrentRow.Cells(GC.SubAccNo).Value) & ".pdf")
        rpt.ReportDoc.ExportToDisk(CrystalDecisions.Shared.ExportFormatType.PortableDocFormat, fileName)
        Process.Start("explorer.exe", "/select," & fileName)
        Process.Start("whatsapp://send?phone=2" & bm.ExecuteScalar("select Tel from customers where Id=" & Val(G.CurrentRow.Cells(GC.SubAccNo).Value))) '& "&Text=" & System.Net.WebUtility.UrlEncode(bm.ExecuteScalar("select WhatsAppNotes from Stores where Id=" & Val(StoreId.Text)))
