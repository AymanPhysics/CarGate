...
A:
    ...
    Sub SetImage(Img As ImageBrush, MyUri As String)
        Try
            Dim bi3 As New BitmapImage
            bi3.BeginInit()
            bi3.UriSource = New Uri("pack://application:,,,/CarGate;component/Images/" & MyUri, UriKind.RelativeOrAbsolute)
            bi3.EndInit()
            Img.ImageSource = bi3
        Catch ex As Exception
            If ex.InnerException Is Nothing Then ShowMSG(ex.ToString().Trim) Else ShowMSG(ex.InnerException.Message)
...
A:
    ...
    Public Function SendWhatsAppLocal(phone As String, Optional text As String = Nothing) As Boolean
        Process.Start("whatsapp://send?phone=2" & phone)
        'Process.Start("whatsapp://send?phone=2" & phone & "&Text=" & System.Net.WebUtility.UrlEncode(text))
        'Process.Start("https://api.whatsapp.com/send/?phone=2" & phone & "&text= ")
        'Process.Start("https://api.whatsapp.com/send/?phone=2" & phone & "&text=" & System.Net.WebUtility.UrlEncode(text) & "&type=phone_number&app_absent=0")

        Dim zero = IntPtr.Zero
        zero = FindWindow(Nothing, "WhatsApp")
        If zero = IntPtr.Zero Then
            zero = FindWindow(Nothing, "WhatsApp Beta")
        End If
        If zero <> IntPtr.Zero Then
            SetForegroundWindow(zero)
            Thread.Sleep(3000)
            Forms.SendKeys.SendWait(text)
